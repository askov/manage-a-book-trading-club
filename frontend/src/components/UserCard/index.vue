<template>
  <transition name="fade"
    ...
    <div class="user-card rounded bg-white p-1 d-flex mb-1 shadow">
      <router-link :to="{name: 'user-details', params:{userId: user.id}}"
      ...
      <div class="text-secondary ml-2 d-flex align-items-center">
        <router-link :to="{name: 'user-books', params: {userId: user.id}}">
          <b-badge variant="primary">books: {{user.books_added}}</b-badge>
        </router-link>
      </div>
    </div>
  </transition>
